...
public class BlockDecompressor extends JACBlock implements ITileEntityProvider {
	...
	public TileEntity createNewTileEntity(World worldIn, int meta) {
		return new TileEntityDecompressor();
	}
	...
	public void onBlockPlacedBy(World worldIn, BlockPos pos, IBlockState state, EntityLivingBase placer,
		...
		if (stack.hasDisplayName()) {
			...
			if (tileentity instanceof TileEntityDecompressor) {
				((TileEntityDecompressor) tileentity).setCustomInventoryName(stack.getDisplayName());
			}
		}
	}
	...
	public int getMetaFromState(IBlockState state) {
		return ((EnumFacing) state.getValue(FACING)).getIndex() + (state.getValue(ACTIVE) ? 5 : 0);
	}
